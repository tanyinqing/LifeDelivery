...
public class BaseWeb<T> {
    ...
    //郭登晋 的模拟服务器
    protected static String GuoBaseUrl = "http://211.149.169.221:8080" + "/umijoyappsvr";
    //真正的服务器
    ...
    protected void parse(String json, Type type, DataListener<T> listener) {
        ...
        try {
			...
			/*	System.out
                ...
                listener.onSuccess(obj);//这是返回的回调的数据
                listener.onFailed(obj);
            } else {
                switch (mPrefUtil.getIntSetting("tiShi")){
                    case 0:
                        PublicUtil.ShowToast("访问网络失败，请重新访问");
                        mPrefUtil.putSetting("tiShi",10);
                        break;
                    default:
    ...
    protected void post(String url, RequestParams params,
        ...
        showProgressDialog();
        getHttpUtil().send(HttpRequest.HttpMethod.POST, url, params,
                new RequestCallBack<String>() {
                    ...
                    public void onFailure(HttpException arg0, String arg1) {
                        ...
                        if (mPrefUtil.getIntSetting("PhoneCode")==1){
                            mPrefUtil.putSetting("PhoneCode",2);
                        }else {
